...
VERSION="v0.3.6"  # version of this setup script

# Move to the folder that this bash script resides, so later commands can use relative paths
...
############################################################
declare -a CONT_NAMES=("cgn")
declare -a IMG_NAMES=("kolinguo/cgn:IMG_TAG")
declare -a DOCKERFILE_PATHS=("./docker/Dockerfile")
# User name inside docker, if user does not exist, the user will be created
# (with the same user/group id as host $USER).
declare -a USER_NAMES=("root")
...
evaluate_strictness()
{
  [[ "$2" =~ ^-(-(container-name|tag|work-dir|build-local|push|build-only|rm|bashrc-only|help|version)$|[twlpohV]) ]] && die "You have passed '$2' as a value of argument '$1', which makes it look like that you have omitted the actual value, since '$2' is an option accepted by this script. This is considered a fatal error."
}
...
print_help()
{
  local all_cont_names=$(printf "'\e[1;36m%s\e[0m' " "${CONT_NAMES[@]}")  # ]]
  local usage="Usage: $(basename ${0}) [-t <tag>] [-w <work-dir>] [OPTION]... <container-name>\n"
...
# Custom bashrc, modified from /etc/bash.bashrc on Ubuntu, appended to ~/.bashrc
...
build_docker_image() {
  local build_args=""
  # Update WORKDIR
  build_args+=" --build-arg WORKDIR=${_arg_work_dir} "

  # Build and run the image
  echo -e "\nBuilding image '${IMGNAME}'..."
  docker build -f "$DOCKERFILEPATH" -t "$IMGNAME" $build_args --network=host .

  echo -e "\nCreate a temporary container to add custom.bashrc and user"
  local temp_cont_name="temp_${CONTNAME}"
  if [ 1 -eq $(docker ps -a | grep -w "${temp_cont_name}$" | wc -l) ] ; then
    echo -e "\e[31mError: existing container '${temp_cont_name}' found, please remove it\e[0m" >&2  # ]]
    exit 1
  fi
  docker create -it --rm --name="$temp_cont_name" -u 0:0 "$IMGNAME" /bin/bash
  docker start "$temp_cont_name"

  # If USERNAME does not exist
  if [[ "$(docker exec -it "$temp_cont_name" sh -c "id -u ${USERNAME}")" = *"no such user"* ]] ; then
    # If a group with the same group(host) id does not exist
    if [ -n "$(docker exec -it "$temp_cont_name" sh -c "getent group `id -g` >/dev/null || echo False")" ] ; then
      docker exec -it "$temp_cont_name" sh -c "groupadd -g `id -g` ${USERNAME}"
    else
      docker exec -it "$temp_cont_name" sh -c "groupmod -n ${USERNAME} \$(getent group `id -g` | cut -d: -f1)"
    fi

    # If a user with the same user(host) id does not exist
    if [[ "$(docker exec -it "$temp_cont_name" sh -c "id -u `id -u`")" = *"no such user"* ]] ; then
      docker exec -it "$temp_cont_name" sh -c "useradd -s /bin/bash -u `id -u` -g `id -g` -m ${USERNAME}"
    else
      docker exec -it "$temp_cont_name" sh -c "usermod -l ${USERNAME} \$(id -nu `id -u`)"
      docker exec -it "$temp_cont_name" sh -c "usermod -s /bin/bash -d /home/${USERNAME} -g `id -g` -m ${USERNAME}"
    fi
  fi

  # If USERNAME is not root, add USERNAME to sudoers with no password required
  if [ "$USERNAME" != "root" ] ; then
    # Make sure sudo is installed
    docker exec -it "$temp_cont_name" sh -c "apt-get update && apt-get install -y sudo && rm -rf /var/lib/apt/lists/*"
    # Add USERNAME to sudoers
    docker exec -it "$temp_cont_name" sh -c "echo ${USERNAME} ALL=\(root\) NOPASSWD:ALL > /etc/sudoers.d/${USERNAME}"
    docker exec -it "$temp_cont_name" sh -c "chmod 0440 /etc/sudoers.d/${USERNAME}"
  fi

  # Create container with USERNAME
  CONTAINER_ARGS+=" -u ${USERNAME}:${USERNAME} "

  # Fetch ~/.bashrc from image, append custom.bashrc, and copy back into the image
  echo -e "\nAppend custom.bashrc to image '${IMGNAME}'..."
  local home_path="$(docker exec -it "$temp_cont_name" sh -c "getent passwd ${USERNAME} | cut -d: -f6")"
  home_path="$(echo "$home_path" | sed 's/\r$//')"
  docker cp "${temp_cont_name}:${home_path}/.bashrc" user.bashrc
  cat custom.bashrc >> user.bashrc
  docker cp user.bashrc "${temp_cont_name}:${home_path}/.bashrc"
  docker commit -m "Append custom.bashrc" "$temp_cont_name" "$IMGNAME"
  docker rm -f "$temp_cont_name"
  rm -rfv custom.bashrc user.bashrc
}

build_docker_container() {
  # Build a container from the image
  echo -e "\nRemoving older container ${CONTNAME}..."
  if [ 1 -eq $(docker ps -a | grep -w "${CONTNAME}$" | wc -l) ] ; then
  ...
  #   GPU related
  #   Use host network and IPC / PID namespace (shared memory segments)
  #   other container arguments (e.g., --user)
  ...
  # Add xhost for GUI
  xhost +local:$(docker inspect --format='{{ .Config.Hostname }}' "$CONTNAME")
}

start_docker_container() {
  docker start -ai "$CONTNAME"

  if [ 0 -eq $(docker ps -a | grep -w "${CONTNAME}$" | wc -l) ] ; then
...
############################################################
# Checks on arguments
check_cont_name  # check if _arg_container_name can be found in CONT_NAMES
