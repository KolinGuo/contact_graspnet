...
def farthest_points(
    ...
    for iter in range(nclusters):
        ...
        if verbose:
            print(f"farthest points max distance : {np.max(distances)}")
...
def depth2xyz(depth_image, intrinsics) -> np.ndarray:
    """Use camera intrinsics to convert depth_image to xyz_image
    :param depth_image: [H, W] or [H, W, 1] np.floating/np.uint16 np.ndarray
...
def load_available_input_data(p, K=None):
    ...
    if ".np" in p:
        ...
        raise ValueError(f"{p} is neither png nor npz/npy file")
...
class PointCloudReader:
    ...
    def __init__(
        ...
        if use_uniform_quaternions:
            ...
            quaternions = [l[:-1].split("\t") for l in open(quat_path).readlines()]
    ...
    def get_scene_batch(self, scene_idx=None, return_segmap=False, save=False):
        ...
        if save:
            ...
            np.savez(f"results/{scene_idx}_acronym.npz", data)
